...
export default async function ArticlePage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const [article, t] = await Promise.all([
    getArticle(slug, locale),
    getTranslations({ locale, namespace: 'pages.articles' }),
  ])
  if (!article) notFound()
    ...
    <>
      ...
      <main className="flex-1 pt-24 pb-20 px-6">
        <div className="max-w-3xl mx-auto">
          <Link href="/articles" className="text-sm text-[#64748b] hover:text-[#e2e8f0] transition-colors mb-8 inline-block">{t('backAll')}</Link>
          ...
          {article.body && article.body.length > 0 ? (
            ...
            <div className="prose prose-invert max-w-none text-[#94a3b8]">
              <p className="text-[#475569] italic">{t('detailPlaceholder')}</p>
            </div>
          )}
        </div>
      </main>
